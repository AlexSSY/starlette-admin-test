...
class UserModelView(ModelView):
    ...
    fields = [
        ...
        PasswordField('password_confirmation', 'Password Confirmation', required=True, exclude_from_list=True, exclude_from_detail=True),
        'password_hash',
        'created_at',
        'updated_at'
    ]
    ...
    def validate(self, request, data):
        ...
        if (request.state.action == RequestAction.CREATE):
            ...
            if session.query(self.model).where(self.model.email == data['email']).first():
