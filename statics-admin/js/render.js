...
const render = {
  ...
  post_comments_count: function render(data, type, full, meta, fieldOptions) {
    if (data == null) return null_column();
    if (Array.isArray(data) && data.length == 0) return empty_column();
    data = Array.isArray(data) ? data : [data].map((d) => escape(d)).join(",");
    if (type != "display") return data;
    is_not_zero = data > 0;
    return `<span class="align-middle d-inline-block text-truncate ${is_not_zero ? 'm-1 py-1 px-2 badge bg-blue-lt lead' : ''}" data-toggle="tooltip" data-placement="bottom" title='${data}' style="max-width: 30em;">${data}</span>`;
  },
};
